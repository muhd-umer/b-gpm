...
from rewardbench_utils import (
    CustomLeftPadRewardBenchPipeline,
    generate_high_dim_result,
    generate_high_dim_result_with_prompt,
)
...
def get_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to use flash attention. Default to False",
    )
    parser.add_argument(
        "--is_custom_model",
        action="store_true",
        default=False,
        help="Use custom defined model, Default to False",
    )
    parser.add_argument(
        "--value_head_dim",
        type=int,
        default=2,
        help="Dimension of the value head in the general preference model. Ignored by the Bradley Terry model. Should be even.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="model",
        help="Model name. Default to be 'model'",
    )
    parser.add_argument(
        "--add_prompt_head",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=42,
        help="Random seed for reproducibility (especially for Bayesian GPM sampling).",
    )
...
def main():
    ...
    if not custom_dialogue:  # not needed for PairRM / SteamSHP
        tokenizer.truncation_side = (
            "left"  # copied from Starling, but few samples are above context length
        )
        ...
        def custom_collate_fn(batch):
            ...
            if isinstance(batch[0]["text_chosen"][0], dict):
                ...
                return default_collate(
                    batch
                )  # Use the default collate behavior otherwise
            ...
            if model_type == "Custom Classifier":
                if not args.is_custom_model:
                    ...
                    if hasattr(reward_pipe.model, "prompt_head"):
                        chosen_reward, prompt_hidden_state = reward_pipe(
                            text_chosen, return_prompt=True, **reward_pipeline_kwargs
                        )
                    else:
                        chosen_reward = reward_pipe(
                            text_chosen, return_prompt=False, **reward_pipeline_kwargs
                        )
                    reject_reward = reward_pipe(
                        text_rejected, return_prompt=False, **reward_pipeline_kwargs
                    )
                    score_chosen_batch = (
                    ...
                    if args.is_general_preference or args.is_bayesian_gpm:
                        if args.value_head_dim == 2 and not hasattr(
                            ...
                            [
                                (
                                    results.append(1)
                                    if chosen[0] * rejected[1] - chosen[1] * rejected[0]
                                    > 0
                            ...
                            result = generate_high_dim_result(
                                args.value_head_dim, chosen_reward, reject_reward
                            )
                            result_batch = (
                                result.float().cpu().detach().numpy().tolist()
                            )
                            [
                            ...
                            result = generate_high_dim_result_with_prompt(
                                reward_pipe.model,
                                args.value_head_dim,
                                chosen_reward,
                                reject_reward,
                ...
                if not (args.is_general_preference or args.is_bayesian_gpm):
                    ...
                    [
                        (
                            results.append(1)
                            if chosen[0] * rejected[1] - chosen[1] * rejected[0] > 0
                            else results.append(0)
                        )
                        for chosen, rejected in zip(
                            score_chosen_batch, score_rejected_batch
    ...
    if not args.pref_sets:
        ...
        average = (
            sum(results_leaderboard.values()) / len(results_leaderboard)
    ...
    save_metrics_for_beaker = (not args.disable_beaker_save) and (not args.do_not_save)
    ...
    results_url = save_to_hub(
        ...
        save_metrics_for_beaker=save_metrics_for_beaker,
    )
    ...
    if not model_type == "Custom Classifier":  # custom classifiers do not return scores
        ...
        sub_path_scores = (
            "eval-set-scores/" if not args.pref_sets else "pref-sets-scores/"
        )
