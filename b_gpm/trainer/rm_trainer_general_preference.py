...
from b_gpm.utils.custom_distributed_sampler import GroupDistributedSampler
...
class GeneralPreferenceRewardTrainer(ABC):
    ...
    def __init__(
        ...
        if self.is_bayesian_gpm:
            ...
            if value_head_dim == 2 and not self.args.add_prompt_head:
                ...
                assert (
                    value_head_dim % 2 == 0
                ), "Dimension of value head for general preference model can not be odd!"
                if self.args.add_prompt_head:
                    self.loss_fn = HighDimGeneralPreferenceMoELoss(
                        model=self.model, value_head_dim=value_head_dim, softmax_tau=tau
                    )
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            for (
                ...
                chosen_response_len = (
                    torch.tensor(chosen_response_len)
                    .view(-1, 1)
                ...
                if isinstance(
                    ...
                    preference_loss, prob = self.loss_fn(
                        chosen_reward, reject_reward, margin
                    )

                if args.add_pretrain_loss:
                    if isinstance(self.ptx_loss_fn, DPORefFreeLoss):
                        chosen_output = self.model.forward(
                            chosen_ids, attention_mask=c_mask
                        )
                        ...
                        rejected_output = self.model.forward(
                            reject_ids, attention_mask=r_mask
                        )
                        ...
                        chosen_reward_ptx_loss = self.ptx_loss_fn(
                            ptx_log_probs, ptx_label, c_mask.bool()
                        )

                    loss = (
                        (1 - args.ptx_loss_coef) * preference_loss
                        + chosen_reward_ptx_loss * args.ptx_loss_coef
                    )
                else:
    ...
    def save_logs_and_checkpoints(
        self, args, global_step, step_bar, eval_loss_minimum, logs_dict={}
    ):
                    ...
                    try:
                        if self.strategy.is_rank_0():
                            self.clean_old_checkpoints(
                                args.save_path, args.save_best_model
                            )
                    except Exception as e:
                        self.strategy.print(
                            f"Error during deleting old checkpoint: {str(e)}"
                        )
                        raise
        ...
        if global_step % args.save_steps == 0:
            ...
            self.strategy.save_model(
                self.model, self.tokenizer, os.path.join(args.save_path, tag)
            )
    ...
    def clean_old_checkpoints(self, output_dir, max_checkpoints=3):
        ...
        ]  # If the number of directories exceeds max_checkpoints, delete the oldest one
        ...
        with torch.no_grad():
            ...
            for (
                ...
                if isinstance(self.loss_fn, HighDimGeneralPreferenceRegressionMoELoss):
                    ...
                    prompt_hidden_state = torch.gather(
                        chosen_last_hidden_states, dim=1, index=prompt_len_expanded
                    ).squeeze(1)
                    preference_loss, prob = self.loss_fn(
            ...
            if self._wandb is not None and self.strategy.is_rank_0():
                logs = {
                    "eval/%s" % k: v for k, v in {**logs, "global_step": steps}.items()
                }
                self._wandb.log(logs)
    ...
    def concatenated_forward(
        self, model, chosen_ids, c_mask, reject_ids, r_mask, return_output: bool = False
    ):
