...
def generate_high_dim_result_with_prompt(
    model, value_head_dim, chosen_reward, rejected_reward, prompt_hidden_states
):
    R_matrix = model.create_skew_symmetric_block_matrix(
        value_head_dim, chosen_reward.device, chosen_reward.dtype, prompt_hidden_states
    )
...
class GPMPipeline:
    def __init__(
        ...
        self.add_prompt_head = (
            True if "prompt_head.weight" in combined_weights else False
        )

        cls_class = get_reward_model(
            base_causal_class,
            base_class,
            add_prompt_head=self.add_prompt_head,
        ...
        self.tokenizer = get_tokenizer(
            model_name_or_path, self.model, "left", use_fast=True
        )
    ...
    def __call__(self, samples: List[List[Dict[str, str]]], return_prompt=False):
        ...
        if return_prompt:
            ...
            for i in range(len(input_texts)):
                ...
                chosen_response_len = (
                    chosen_token["attention_mask"].sum()
                    - prompt_token["attention_mask"].sum()
                )
                chosen_response_len_list.append(chosen_response_len)
        chosen_response_len = (
            torch.tensor(chosen_response_len_list).view(-1, 1).to(self.device)
        )
        if return_prompt:
            ...
            prompt_hidden_state = torch.gather(
                chosen_last_hidden_states, dim=1, index=prompt_end_index_expanded
            ).squeeze(1)
...
result = generate_high_dim_result_with_prompt(
    rm.model, rm.value_head_dim, reward1, reward2, prompt_hidden_state
)
# score = result / rm.tau
...
results = []
[results.append(1) if result > 0 else results.append(0) for result in result_batch]
