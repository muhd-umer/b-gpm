...
def ranking(args, prompts, candidates):
    ...
    ranks, raw_table = blender.rank(
        prompts, candidates, return_scores=True, batch_size=1, return_raw_table=True
    )
    # print(ranks.shape)
    # print(ranks[0])
    ...
    np.save(
        f"ranking/{args.output_dir}/{args.gpu}_{args.data_frac}_table.npy", raw_table
    )
