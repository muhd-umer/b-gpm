...
class RewardModel:
    ...
    def pair_wise_scores(self, prompt: List[str], candidates: List[List[str]]):
        ...
        formatted = self.format_pair(
            prompt, candidates
        )  # size of (bsz, num_candidates * (num_candidates-1))
        formatted = self.regroup_formatted(
            formatted
        )  # size of (num_candidates * (num_candidates-1), bsz)
    ...
    def regroup_formatted(self, formatted: List[List[str]]):
        ...
        for i in range(len(formatted[0])):  # i: batch index
            regrouped.append(
                [formatted[j][i] for j in range(len(formatted))]
            )  # j: pair index
        return regrouped
...
def get_tokenizer(
    pretrain, model, padding_side="left", truncation_side="right", use_fast=True
):
        ...
        def custom_forward(
            ...
            if not self.is_general_preference:
                ...
                if self.training:
                    ...
                    reward = F.normalize(
                        reward, p=2, dim=-1
                    )  # Shape will be [batch_size, value_head_dim]
                if return_output:
                    return reward, outputs
                else:
                    return reward, None
...
class CustomPairPreferenceModel(RewardModel):
        ...
        with torch.no_grad():
            ...
            for i in range(len(inputs)):
                ...
                if hasattr(self.model, "prompt_head"):
                    reward_i, output_i = self.model.custom_forward(
                        **input_i, return_output=True
                    )
                    response_len_i = (
                        torch.tensor(response_lengths[i]).view(-1, 1).to("cuda")
                    )
                    ...
                    prompt_end_index_expanded = prompt_end_index.unsqueeze(-1).expand(
                        -1, -1, last_hidden_states_i.size(-1)
                    )
                    prompt_hidden_state = torch.gather(
                        last_hidden_states_i, dim=1, index=prompt_end_index_expanded
                    ).squeeze(1)
                    ...
                    if i != j:
                        ...
                        if hasattr(self.model, "prompt_head"):
                            score = self.calculate_score(
                                reward_i, reward_j, prompt_hidden_state_cache[i]
                            )
                        else:
    ...
    def calculate_score(self, chosen, rejected, prompt_hidden_state=None):
        if self.is_general_preference:
            if self.value_head_dim == 2:
                ...
                score = self.generate_high_dim_result(
                    self.value_head_dim, chosen, rejected
                )
            else:
                score = self.generate_high_dim_result_with_prompt(
                    self.model,
                    self.value_head_dim,
                    chosen,
                    rejected,
    ...
    def pair_wise_scores(self, prompt: List[str], candidates: List[List[str]]):
        ...
        if not self.add_prompt_head:
            ...
            formatted, response_lengths = self.format_pair(
                prompt, candidates
            )  # size of (bsz, num_candidates * (num_candidates-1))
            formatted = self.regroup_formatted(
                formatted
    ...
    def generate_high_dim_result(self, value_head_dim, chosen_reward, rejected_reward):
        R_matrix = torch.zeros(
            (value_head_dim, value_head_dim),
            device=chosen_reward.device,
            dtype=chosen_reward.dtype,
        )
        ...
        if chosen_reward.device == rejected_reward.device == R_matrix.device:
            ...
            result = torch.bmm(
                transformed_chosen.view(chosen_reward.shape[0], 1, value_head_dim),
                rejected_reward.view(rejected_reward.shape[0], value_head_dim, 1),
            )
            result = result.view(chosen_reward.shape[0])
        return result

    def generate_high_dim_result_with_prompt(
        self,
        model,
        value_head_dim,
        chosen_reward,
        ...
        R_matrix = model.create_skew_symmetric_block_matrix(
            value_head_dim,
        ...
        if chosen_reward.device == rejected_reward.device == R_matrix.device:
            transformed_chosen = torch.bmm(
                chosen_reward.view(chosen_reward.shape[0], 1, value_head_dim),
            ...
            for i in range(len(candidates)):
                for j in range(len(candidates)):
                    if i != j:
                        ...
                        context_str = self.tokenizer_plain.apply_chat_template(
                            context, tokenize=False
                        )
    ...
    def get_scores(self, inputs):
        ...
        with torch.no_grad():
            ...
            for batch_inputs in inputs:
                batch_inputs = {
                    k: v.to(self.model.device) for k, v in batch_inputs.items()
                }
