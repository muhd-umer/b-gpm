...
def batch_rm_inference(args):
    ...
    dataset = GeneralRewardDataset(
        dataset, tokenizer, args.max_len, strategy, args.is_custom_dataset
    )
    ...
    if use_general_preference:
        if args.value_head_dim == 2:
            ...
            assert (
                args.value_head_dim % 2 == 0
            ), "Dimension of value head for general preference model can not be odd!"
            loss_fn = HighDimGeneralPreferenceLoss(
                tau=args.general_preference_tau, value_head_dim=args.value_head_dim
            )
            strategy.print(
                "Loss for high-dimensional value head General Preference model."
            )
    else:
    ...
    with torch.no_grad():
        for chosen_ids, chosen_masks, reject_ids, reject_masks, _, _ in pbar:
            ...
            if isinstance(chosen_rewards, BayesianEmbedding):
                chosen_rewards = (
                    chosen_rewards.mean  # deterministic evaluation uses posterior mean
                )
            ...
            if isinstance(reject_rewards, BayesianEmbedding):
                reject_rewards = (
                    reject_rewards.mean  # deterministic evaluation uses posterior mean
                )
