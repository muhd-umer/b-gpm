...
def train(args):
    ...
    if args.train_split_ratio < 1:
        ...
        train_data = total_data.select(
            range(int(total_data_lengh * args.train_split_ratio))
        )
        eval_data = total_data.select(
            range(int(total_data_lengh * args.train_split_ratio), total_data_lengh)
        )
        ...
        if len(eval_data) != 0:
            ...
            strategy.print(
                "No separate validation data split was detected. The entire dataset will be utilized for training."
            )
    else:
        train_dataset = GeneralRewardDataset(
            total_data,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_template", type=str, default="Human:\n{}\nAssistant:\n"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to use custom cyclic dataset. Default to False.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Hyperparameter tau used in general preference loss.",
    )
    parser.add_argument(
        "--group_size",
        type=int,
        default=1,
        help="Number of data to group together during shuffling.",
    )
    parser.add_argument(
        "--value_head_dim",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Include the pretraining loss of chosen inputs in the total loss calculation.",
    )
    parser.add_argument(
        "--ptx_loss_coef",
        type=float,
        default=0.1,
        help="coefficient for pretraining loss included in the total loss.",
    )
    parser.add_argument(
        "--train_split_ratio",
        type=float,
        default=1,
        help="Ratio of the dataset to use for training. (1-train_split_ratio) for validation. Should not exceed 1.",
    )
    parser.add_argument(
        "--reward_scaler_beta",
        type=float,
        default=2.0,
        help="A constant that controls the scaling of the reward difference.",
    )
    parser.add_argument(
        "--reward_margin",
        type=float,
        default=1.0,
        help="Chosen response exceeds rejected reward by at least reward_margin. A hyperparameter for DPORefFree Loss.",
    )
    parser.add_argument(
        "--regression_target_margin",
        type=float,
        default=10.0,
        help="Target regression margin. A hyperparameter for Regression Loss.",
    )
    parser.add_argument(
        "--return_prompt_length",
        action="store_true",
        default=False,
        help="Return the prompt length in the dataset collator if set. Default to False. Should set to be True when 'add_prompt_head' is True.",
    )
    parser.add_argument(
        "--add_prompt_head",
        action="store_true",
    ...
    parser.add_argument(
        "--wandb_project", type=str, default="train_rm_general_preference"
    )
    parser.add_argument(
        "--wandb_run_name",
        type=str,
